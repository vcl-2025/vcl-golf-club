-- 为比赛活动分配报名人员 (每个比赛至少6个已支付完成的报名) - 修正版本
-- 使用正确的 event_registrations 表结构
...
SELECT 
    ...
    u.id,
    up.full_name,
    'M' || LPAD(EXTRACT(EPOCH FROM up.created_at)::INTEGER % 10000, 4, '0'),
    '604-' || LPAD((random() * 9999999)::INTEGER, 7, '0'),
    'paid',
