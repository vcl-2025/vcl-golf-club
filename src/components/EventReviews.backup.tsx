...
export default function EventReviews() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedEvent && (
        <div 
          className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-[10000] pt-20"
          onClick={(e) => {
        ...
        >
          <div 
            className="bg-white rounded-2xl w-full max-w-[1080px] flex flex-col relative mx-auto"
            onClick={(e) => e.stopPropagation()}
            style={{
          ...
          >
            {/* 固定头部 - 测试 */}
            <div 
              className="sticky top-0 bg-red-500 border-b-4 border-blue-500 flex items-center justify-between px-6 py-6 rounded-t-2xl"
              style={{ 
                zIndex: 10002,
                position: 'sticky',
                top: 0,
